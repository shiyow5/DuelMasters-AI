...
export const THRESHOLDS = {
  ...
  /**
   * **システム障害**でツールが落ちた問の件数 (#109)。**1件も許さない。**
   *
   ...
   *
   * **引数エラーは数えない。** モデルが存在しない文明を指定した等は、システムが壊れている
   * わけではなく呼び直せば回復する。混ぜると**モデルの推測ミスでゲートが落ちる**。
   * 根拠が取れているかは evidenceRate が見る (引数エラーは根拠にならない = そちらに出る)。
   *
