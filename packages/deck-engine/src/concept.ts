 ...
 *
 * **素の「無限」ではなく「無限に…する」に絞る (#137、本番 cards の実測に基づく)。**
 * 本番の「無限」を含む36枚を実測すると、**超無限進化 (進化キーワード) 18枚・∞パワーアタッカー
 * (無限大) 3枚・ロック (無限のマナ) 6枚・カード名 (無限皇/無限龍 等)** が占め、**ループ部品は0件**
 * だった。素の「無限」を信号にすると、進化クリーチャーを2種6枚以上積んだ普通の進化デッキが
 * combo 扱いになり、受け/フィニッシャーの減点が不当に緩和 (過小評価) される。「無限に」は
 * これら (超無限進化/無限大/無限の/名前) をすべて除外し、「無限にマナを生み出す」のような
 * 実際のループ文脈だけを拾う (実マッチは除去カード1枚のみで、1種では combo 発火条件に届かず無害)。
 */
const COMBO_SIGNAL =
  /無限に|好きなだけ|繰り返|このターン.{0,15}(もう一度|追加のターン)|次のターン.{0,10}追加/;
