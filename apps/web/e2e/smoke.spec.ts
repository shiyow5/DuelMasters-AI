...
test("トップ(チャット)が表示され、空入力では送信ボタンが disabled", async ({
  page,
}) => {
  await page.goto("/");
...
test("/deck でデッキを評価すると 30/100 が表示される (DB無し固定スコア)", async ({
  page,
}) => {
...
test("Delete Chat が confirm 経由で履歴を空にする (0件でも UI が壊れない)", async ({
  page,
}) => {
  await page.goto("/");
