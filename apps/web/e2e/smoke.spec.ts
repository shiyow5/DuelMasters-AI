 ...
 * 会話を閉じて新規状態に戻す (#110)。
 *
 * 以前は「Delete Chat」で confirm を出してローカル state を空にしていた。会話を DB に
 * 保存するようになったので、**ヘッダのボタンは DB からは消さない** (新規状態に戻すだけ)。
 * 会話そのものを消すのはサイドバーの削除ボタン — 誤操作で会話を失わせないため、
 * 「閉じる」と「消す」を分けた。
 */
test("「新しい会話」で新規状態に戻る (0件でも UI が壊れない)", async ({ page }) => {
  await page.goto("/");
  await page.locator('button[title="新しい会話"]').click();
  await expect(page.getByText("DM AI Master へようこそ")).toBeVisible();
});
