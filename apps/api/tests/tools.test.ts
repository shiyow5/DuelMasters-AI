...
describe("executeToolCall search_cards バリデーション (単体)", () => {
  it("不正な civilization は throw せずエラーメッセージを返す", async () => {
    const result = await executeToolCall("search_cards", {
      query: "x",
      civilization: "purple",
    });
    expect(result).toContain("ツール引数が不正です");
  });
});

describe.skipIf(!hasTestDb)("executeToolCall フィルタ (統合)", () => {
  ...
  it("civilization フィルタが効く", async () => {
    const r = await executeToolCall("search_cards", {
      query: "ドラゴン",
      civilization: "fire",
    });
    expect(r).toContain("火の1");
    expect(r).not.toContain("水の1");
  });
  ...
  it("type フィルタが効く", async () => {
    ...
    expect(r).toContain("水の1");
    expect(r).not.toContain("火の1");
  });
});
