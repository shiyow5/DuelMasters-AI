 ...
 */
export function buildRefutePrompt(rulingText: string, article: RuleArticle, claim: string): string {
...
export async function runAuditRulings(
  ...
  await mapPool(candidates, CONCURRENCY, async (ruling) => {
    ...
    console.log(
      `  ★ qa_id ${ruling.qaId} は ${verdict.article} と矛盾: ${ruling.question.slice(0, 40)}`,
    );
  });
