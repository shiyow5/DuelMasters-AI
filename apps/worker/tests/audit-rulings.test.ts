...
describe("verifyGrounding", () => {
  ...
  it("相互参照にたまたま一致しただけの条文を親として採らない", () => {
    // 総合ルールは条文どうしで相互参照する (「704.3 を参照」)。条番号のラベルが本文に
    // 出てくるチャンクを無条件に親にすると、**引用元と無関係なチャンク**を親に選んでしまい、
    // article と quote の対応が壊れる (article は 704.3 なのに quote は別条文の文言、という状態)。
    // 条番号と引用が同じチャンクに載っていることを要求する。
    const crossRef: RuleArticle[] = [
      // 704.3 のラベルだけが出てくるチャンク (引用元ではない)
      { article: "601.1", text: "601.1. 詳しくは 704.3 を参照してください。" },
      // 引用の実際の出どころ。ただし 704.3 のラベルは含まない
      { article: "900.1", text: "900.1. まったく別の規定がここに書かれています。" },
    ];
    const v = verifyGrounding(
      verdict({ article: "704.3", quote: "まったく別の規定がここに書かれています" }),
      crossRef,
      RULING,
    );
    expect(v.ok).toBe(false);
