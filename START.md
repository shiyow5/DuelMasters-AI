...
### 7-5b. 現行ルールと矛盾する古い裁定のクレンジング (#92)

公式サイトの裁定 Q&A (qa_old) には改定前の回答が残っていることがあり、RAG がこれを引くと
回答が汚れます。

#### この問題は大部分が既に解決しています

Issue #92 が名指しした qa_id 34932 (「トリガー能力を使ってからアンタップする」= 総合ルール
501.1 → 501.2 と逆) は、**すでに corpus に存在しません**。公式サイト自身が同じ質問で訂正版
(qa_id 37341: アンタップ → 誘発 → ドロー) を出し直しており、#82 の重複排除 (質問文で名寄せし、
新しい qa_id を残す) が古い方を落としているためです。

つまり「改定前の裁定」は **公式の訂正 + 重複排除で構造的に処理済み**です。

#### LLM による矛盾検出は失敗しました (使わないでください)

`audit:rulings` を実装して試しましたが、**判定は使い物になりません**。実測 (2026-07-14):

| 反証プロンプト           | 結果                                               |
| ------------------------ | -------------------------------------------------- |
| 「迷ったら両立と答えろ」 | 検出率 0%。**本物の矛盾 34932 すら取り落とす**     |
| 「手順に違いが出るか」   | 106件中 78件 (**74%**) を矛盾と判定 = 偽陽性の氾濫 |
...
- qa_id 36089「覚醒したクリーチャーは召喚酔いがなくなります」
  ...
  → **完全に一致しているのに「矛盾」と判定**
- 「超次元ゾーンとは？」「∞ブレイカーとは？」などの**用語説明**を軒並み矛盾と判定

逐語引用の機械照合 (`verifyGrounding`) は**捏造**を止められますが、**見当違いの判定**は
止められません。引用自体は実在するからです。

信頼できる検出には**機械的なオラクル**が要ります。「裁定どおりに処理した結果」と「条文どおりに
処理した結果」を実際に実行して突き合わせる —— それがゲームエンジン (#101) です。

```bash
# プロンプトを触ったときの退行検知だけは有効 (正解の分かった2件を回す)
pnpm --filter @dm-ai/worker audit:rulings -- --control

# 降格の反映。DEPRECATED_RULINGS は現在**空**なので、実行しても何も起きない
pnpm --filter @dm-ai/worker deprecate:rulings
```

#### 降格の仕組み自体は動きます (信頼できる検出ができたとき使う)

`apps/worker/src/data/deprecated-rulings.ts` に載せた裁定だけが `chunk_meta.deprecated` を
立てられ、RAG 検索 (キーワード経路・ベクトル経路の両方) から除外されます。
**行は削除せず印を立てるだけなので、一覧から1行消して流し直せば元に戻ります。**
裁定取込は qa_id 単位の DELETE+INSERT で印が消えるため、`ingest:rulings` が最後に一覧を
貼り直します (これが無いと週次 cron のたびに矛盾裁定が RAG へ復活します)。

ここに載せてよいのは、**人が総合ルールの条文を読んで確認したもの**だけです。
