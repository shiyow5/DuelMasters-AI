...
公式サイトの裁定 Q&A (qa_old) には**改定前の回答がそのまま残っています**。RAG がこれを引くと
回答が汚れます。実例: qa_id 34932 は「トリガー能力をすべて使ってからアンタップする」と答え
ますが、総合ルールは 501.1 (アンタップ) → 501.2 (「ターンのはじめに」誘発) → 502.1 (ドロー)
の順と定めており、**順序が逆**です。

対策は2段構えです。

```bash
# 1. 検出: 総合ルールと突き合わせて矛盾候補を出す (DB は変更しない)
#    カード名を含まない「一般ルール裁定」だけを見る (3200件 → 611件)。
#    カード個別の裁定はカードのテキスト解釈であって、条文と正面から矛盾することは稀。
pnpm --filter @dm-ai/worker audit:rulings -- --out=/tmp/audit.json

# 2. 反映: レビュー済みの一覧を DB の chunk_meta.deprecated に立てる
pnpm --filter @dm-ai/worker deprecate:rulings
```

**LLM の判定だけでは落としません。** 過去に judge はカードの実在性も DM のルールも誤りました。
`audit:rulings` は LLM に「どの条番号の・どの文言と矛盾するか」を**逐語引用**で答えさせ、
その条番号が実在し引用が本当にその条文の部分列であることを**機械的に検証**します
(`verifyGrounding`)。捏造した条番号や言い換えた「引用」はここで落ちます。
そのうえで**人がレビュー**し、`apps/worker/src/data/deprecated-rulings.ts` に載せたものだけが
実際に降格されます。

行は削除せず印を立てるだけなので、**一覧から1行消して流し直せば元に戻ります**。
裁定取込は qa_id 単位の DELETE+INSERT なので印は毎回消えますが、`ingest:rulings` が
最後に一覧を貼り直すため、週次 cron のあとも印は残ります。
